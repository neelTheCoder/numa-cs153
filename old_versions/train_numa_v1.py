...
PLOT_DIR           = Path("enigma_plots_v1")

MODEL_TAG   = "v1 (TinyENIGMA depthwise-CNN + eeg_channel_weights)"
CKPT_PREFIX = "enigma_v1"
...
def main(args):
    ...
    for fold in folds:
        ...
        tqdm.write(f"\n  Fold {fold} top-1 acc: {acc:.3f}  (chance = {1/N_CLASSES:.3f})")
        tqdm.write(classification_report(labels, preds, target_names=target_names,
                                         digits=3, zero_division=0))
        if curves:
            plot_training_curves(fold, curves, acc, PLOT_DIR)
        ...
        torch.save({"model_state": best_state, "label_map": label_map,
                    ...
                    "resample_hz": RESAMPLE_FREQ}, ckpt)
